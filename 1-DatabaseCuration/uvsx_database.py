...
stage_uniprot_protein(accession, f'gold_standards')


# Use search terms to query Uniprot and save sequences found along with all avaliable metadata
QuerySearchUniprot("(recA OR uvsX OR recombinase) AND taxonomy_id:10239", raw_seqs, raw_meta)

sleep(2)

# Use search terms to query NCBI databases, saving sequences along with all avaliable metadata
QuerySearchNCBI("(uvsx OR recA) AND txid10239[Organism:exp]", raw_seqs, raw_meta)
...
InterproSearchUniprot(interpro_ids, raw_seqs, raw_meta)
...
reference=read_fasta(f'gold_standards/P04529.fasta') #getting protein sequence of UvsX reference
CreateMutants(mutants, reference, 'gold_standards') #Creating reference sequences and writing them to a fasta file
...
pdb_ids=['7Z3M','9GBG']
IDSearchRCSB(pdb_ids, 'gold_standards')
