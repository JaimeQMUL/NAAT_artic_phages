...
hits=pd.read_csv('uvsx/data/curated_database/filtered_metadata.csv')
print(f'Number of accessions: {len(hits)}')

# Adding the Results from HMMs to hits
results_dir='uvsx/data/hmms/results/'
...
# Finding walker A
a_results, a_present=MotifDatabaseSearch(hits['accession'], 'uvsx/data/curated_database/cleaned_curated_database.fasta', r"[GA]....[GF]K[TS]")
print(f'Walker A found {sum(a_present)}/{len(hits)}')
hits['Walker_A']=a_present

# Finding walker B
b_results, b_present=MotifDatabaseSearch(hits['accession'], 'uvsx/data/curated_database/cleaned_curated_database.fasta', r"[AVILMFWY]{4}DE?")
print(f'Walker B found {sum(b_present)}/{len(hits)}')
hits['Walker_B']=b_present

#Writing it to a csv for later
hits.to_csv('uvsx/data/scoring/filtered_scoring.csv', index=False)
...
WriteCleanedFasta(top_hits, 'uvsx/data/curated_database/top_hits.fasta', 'uvsx')
print(f'{len(top_hits)}/{len(hits)} Written to fasta')

# Writing accessions to a csv for later
top_hits.to_csv('uvsx/data/curated_database/top_hits.csv', index=False)